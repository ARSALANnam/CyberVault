...
import ui.panels.GeneratorPanel;
...
//    StrengthMeter meter;
    ...
    JPanel buildTitleBar() {
        ...
        JButton settingsBtn = miniBtn("SETTINGS", ev -> openSettings(), ThemeManager.NEON_CYAN);
        settingsBtn.setPreferredSize(new Dimension(96, 22));
        btns.add(settingsBtn);
        btns.add(miniBtn("\u2500", ev -> setState(JFrame.ICONIFIED), ThemeManager.NEON_CYAN));
        btns.add(miniBtn("\u2715", ev -> System.exit(0), ThemeManager.NEON_PINK));
    ...
    JButton miniBtn(String txt, ActionListener al, Color hover) {
        ...
        b.setForeground(ThemeManager.TXT_DIM);
        ...
        b.addMouseListener(new MouseAdapter() {
            ...
            public void mouseExited(MouseEvent e)  { b.setForeground(ThemeManager.TXT_DIM); }
        });
    ...
    JPanel buildAuthScreen() {
        ...
        JLabel brand = label("C Y B E R V A U L T", ThemeManager.F_BIG, ThemeManager.NEON_CYAN);
        brand.setHorizontalAlignment(SwingConstants.CENTER);
        g.gridy++; g.insets = new Insets(12, 0, 0, 0);
        form.add(brand, g);

        JLabel tag = label("// LOCAL  \u2022  AES-256  \u2022  ZERO CLOUD", ThemeManager.F_MONO_S, ThemeManager.TXT_DIM);
        tag.setHorizontalAlignment(SwingConstants.CENTER);
        g.gridy++; g.insets = new Insets(6, 0, 0, 0);
        form.add(tag, g);

        g.anchor = GridBagConstraints.WEST;
        authTitle = label("ACCESS VAULT", ThemeManager.F_MONO_B, ThemeManager.NEON_GRN);
        g.gridy++; g.insets = new Insets(26, 0, 3, 0);
        form.add(authTitle, g);
        authSub = label("// enter master key to decrypt", ThemeManager.F_MONO_S, ThemeManager.TXT_DIM);
        g.gridy++; g.insets = new Insets(0, 0, 14, 0);
        form.add(authSub, g);

        g.gridy++; g.insets = new Insets(4, 0, 4, 0);
        form.add(label("MASTER KEY", ThemeManager.F_MONO_S, ThemeManager.TXT_DIM), g);
        authPass = passField();
        g.gridy++; g.insets = new Insets(0, 0, 10, 0);
        form.add(authPass, g);

        authLbl2 = label("CONFIRM MASTER KEY", ThemeManager.F_MONO_S, ThemeManager.TXT_DIM);
        ...
        authBtn = new CyberButton("\u25B6 ACCESS VAULT", ThemeManager.NEON_CYAN, true);
        authBtn.setPreferredSize(new Dimension(0, 46));
        authBtn.addActionListener(ev -> authAction());
        g.gridy++;
        form.add(authBtn, g);

        authStatus = label(" ", ThemeManager.F_MONO_S, ThemeManager.NEON_PINK);
    ...
    JPanel buildVaultSelector() {
        ...
        JLabel title = label("SELECT VAULT", ThemeManager.F_TITLE, ThemeManager.NEON_CYAN);
        ...
        if (manager.vaults.isEmpty()) {
            list.add(label("// no vaults yet — create your first one", ThemeManager.F_MONO_S, ThemeManager.TXT_DIM));
        }

        for (VaultInfo v : manager.vaults) {
            JPanel card = new JPanel(new BorderLayout(12, 0));
            card.setBackground(ThemeManager.BG_PANEL);
            ...
            CyberButton open = new CyberButton("▶ OPEN", ThemeManager.NEON_GRN, false);
            ...
            CyberButton ren = new CyberButton("\u270E", ThemeManager.NEON_CYAN, false);
            ...
            if (true) {
                CyberButton del = new CyberButton("🗑", ThemeManager.NEON_PINK, false);
        ...
        CyberButton addBtn = new CyberButton("+ CREATE NEW VAULT", ThemeManager.NEON_YEL, true);
        addBtn.addActionListener(ev -> showCreateVaultDialog());
        bottom.add(addBtn);

        CyberButton impBtn = new CyberButton("\u21E7 IMPORT", ThemeManager.NEON_CYAN, true);
    ...
    void configureAuthMode() {
        ...
        authTitle.setForeground(exists ? ThemeManager.NEON_GRN : ThemeManager.NEON_YEL);
    ...
    void flashAuthError() {
        authCard.setBorder(BorderFactory.createCompoundBorder(
                BorderFactory.createCompoundBorder(
                        BorderFactory.createLineBorder(ThemeManager.NEON_PINK),
                        BorderFactory.createMatteBorder(0, 4, 0, 0, ThemeManager.NEON_PINK)),
                empty(28, 36, 28, 36)));
    ...
    void startActivityMonitor() {
        ...
        autoLockTimer = new Timer(1000, ev -> {
            ...
            Color color = remain < 60 ? ThemeManager.NEON_PINK : remain < 180 ? ThemeManager.NEON_YEL : ThemeManager.TXT_DIM;
    ...
    JPanel buildSidebar() {
        ...
        navPass = new NavButton("> PASSWORDS", ThemeManager.NEON_CYAN);
        navTok  = new NavButton("> API TOKENS", ThemeManager.NEON_PURP);
        navGen  = new NavButton("> GENERATOR", ThemeManager.NEON_YEL);
        ...
        autoLockLabel = label("// AUTO-LOCK: --:--", ThemeManager.F_MONO_S, ThemeManager.TXT_DIM);
     statsLabel = label("\u2026", ThemeManager.F_MONO_S, ThemeManager.TXT_DIM);
        JLabel foot = label("LOCAL ONLY // NO CLOUD", ThemeManager.F_MONO_S, new Color(0x454B6E));
        JPanel sp = new JPanel(new GridLayout(0, 1, 0, 5));
        sp.setOpaque(false);
        sp.add(statsLabel); sp.add(autoLockLabel); sp.add(foot);
        CyberButton lock = new CyberButton("// LOCK VAULT", ThemeManager.NEON_PINK, false);
    ...
    JPanel buildTokensPanel() {
        ...
        JPanel head = new JPanel(new BorderLayout());
        head.setOpaque(false);
        head.add(sectionHeader("API TOKENS", "// hugging face \u2022 openai \u2022 github \u2022 \u2026", ThemeManager.NEON_PURP), BorderLayout.CENTER);
        CyberButton add = new CyberButton("+ NEW TOKEN", ThemeManager.NEON_PURP, true);
        add.setPreferredSize(new Dimension(160, 38));
        add.addActionListener(ev -> openTokenDialog(null));

        CyberButton favTok = new CyberButton(showFavTok ? "\u2605 FAVORITES" : "\u2606 FAVORITES", ThemeManager.NEON_YEL, false);
    ...
    JPanel buildTokenCard(TokenEntry e) {
        ...
        JPanel ttl = new JPanel(new GridLayout(0, 1, 0, 2));
        ttl.setOpaque(false);
        ttl.add(label(e.name.toUpperCase(), ThemeManager.pickMono(Font.BOLD, 14f), ThemeManager.NEON_PURP));
        ttl.add(label("ADDED " + fmtDate(e.created), ThemeManager.F_MONO_S, new Color(0x555C82)));
        head.add(ttl, BorderLayout.CENTER);
        ...
        show.addActionListener(ev -> {
            ...
            tv.setForeground(vis[0] ? ThemeManager.NEON_GRN : ThemeManager.NEON_PINK);
            show.setText(vis[0] ? "HIDE" : "SHOW");
        });
    ...
    void openTokenDialog(TokenEntry ex) {
        JDialog d = cyberDialog(ex == null ? "NEW API TOKEN" : "EDIT API TOKEN", ThemeManager.NEON_PURP);
        ...
        tokRow.setBackground(ThemeManager.BG_PANEL);
        ...
        JPanel btns = new JPanel(new FlowLayout(FlowLayout.RIGHT, 10, 0));
        btns.setOpaque(false);
        CyberButton save = new CyberButton("SAVE TOKEN", ThemeManager.NEON_PURP, true);
        CyberButton cancel = new CyberButton("CANCEL", ThemeManager.TXT_DIM, false);
        save.addActionListener(ev -> {
...
//
//        genOut = field();
//        genOut.setFont(ThemeManager.pickMono(Font.BOLD, 17f));
//        g.gridy = 0; body.add(genOut, g);
//
//        meter = new StrengthMeter();
//        g.gridy = 1; g.insets = new Insets(14, 0, 4, 0);
//        body.add(meter, g);
//        meterLabel = label(" ", ThemeManager.F_MONO_S, ThemeManager.TXT_DIM);
...
//        genLenVal = label("18", ThemeManager.F_MONO_B, ThemeManager.NEON_YEL);
//        genLenVal.setPreferredSize(new Dimension(34, 18));
//        lenRow.add(label("LENGTH", ThemeManager.F_MONO_S, ThemeManager.TXT_DIM), BorderLayout.WEST);
...
//                g2.setColor(ThemeManager.NEON_YEL);
...
//        CyberButton genBtn = new CyberButton("\u26A1 GENERATE", ThemeManager.NEON_YEL, true);
//        genBtn.setPreferredSize(new Dimension(220, 44));
//        CyberButton copyBtn = new CyberButton("COPY", ThemeManager.NEON_GRN, false);
...
//        JDialog d = cyberDialog("CONFIRM ACTION", ThemeManager.NEON_PINK);
...
//        btns.setOpaque(false);
//        boolean[] r = { false };
//        CyberButton yes = new CyberButton("CONFIRM", ThemeManager.NEON_PINK, true);
//        CyberButton no = new CyberButton("CANCEL", ThemeManager.NEON_CYAN, false);
...
//        JDialog d = cyberDialog("SYSTEM ALERT", ThemeManager.NEON_PINK);
...
//    }
    ...
    void openSettings() {
        JDialog d = cyberDialog("SETTINGS", ThemeManager.NEON_CYAN);
        JPanel body = new JPanel(new BorderLayout(0, 20));
        body.setBackground(ThemeManager.BG_PANEL);
        body.setBorder(empty(24, 26, 24, 26));

        body.add(label("// SELECT THEME", ThemeManager.F_MONO_B, ThemeManager.TXT_DIM), BorderLayout.NORTH);
        ...
        for (Theme t : ThemeManager.PRESETS) {
            ...
            pLbl.setFont(ThemeManager.F_MONO_S); pLbl.setForeground(t.txtDim);
            ...
            pCard.add(label("TITLE", ThemeManager.pickMono(Font.BOLD, 10f), t.txt), BorderLayout.NORTH);
            pCard.add(label("••••••••", ThemeManager.F_MONO_S, t.neonPink), BorderLayout.CENTER);
            ...
            card.addMouseListener(new MouseAdapter() {
                public void mouseClicked(MouseEvent e) {
                    ...
                    ThemeManager.applyTheme(t.name);
                    buildFrame();
                    d.dispose();
                ...
                public void mouseEntered(MouseEvent e) {
                    card.setBorder(BorderFactory.createCompoundBorder(
                        BorderFactory.createLineBorder(ThemeManager.NEON_CYAN, 2), empty(12, 14, 12, 14)));
                }
        ...
        scroll.getViewport().setBackground(ThemeManager.BG_PANEL);
    ...
    static JComponent miniChip(String text, Color c, Color bg) {
        JComponent p = new JComponent() {
            ...
            protected void paintComponent(Graphics g) {
                ...
                g2.setFont(ThemeManager.pickMono(Font.BOLD, 8f));
    ...
    public static boolean saveVault() {
